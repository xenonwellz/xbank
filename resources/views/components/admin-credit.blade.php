<div class="p-3 bg-white rounded-lg">
    <div class="flex">
        <div class="inline-flex items-center justify-center w-10 h-10 text-green-800 bg-green-200 rounded-lg">
        ...
        <div class="flex-grow px-3">
            @if ($transaction->medium === 'deposit')
                <h2 class="font-semibold">Cash Deposit</h2>
                <h3 class="text-sm text-gray-700">Processed cash deposit for <span
                        class="font-semibold">{{ $transaction->user->name }}'s</span> account -
                    {{ $transaction->created_at->diffForHumans() }}
                </h3>
            @else
                <h2 class="font-semibold">Cash Transfer</h2>
                <h3 class="text-sm text-gray-700">Transfered cash to <span
                        class="font-semibold">{{ $transaction->user->name }}'s</span> account -
                    {{ $transaction->created_at->diffForHumans() }}
                </h3>
            @endif
    ...
    <div class="flex-shrink-0 w-full mt-4 rounded-lg bg-green-50">
        <div x-data="{ open: false }">
            ...
            <div x-show="open" class="p-3 text-sm font-semibold border-t border-green-500 rounded-b-lg">
